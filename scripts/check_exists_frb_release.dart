...
///   - libsignal_version: The upstream libsignal version
...
void main(List<String> args) async {
  ...
  final releaseTag = 'libsignal_frb-$version';

  logInfo('Checking release status...');
  logInfo('Crate version: $version');

  logInfo('libsignal version: $upstreamVersion');
...
Future<bool> _checkReleaseExists(String tag) async {
  ...
  try {
    final url = Uri.parse(
      'https://api.github.com/repos/djx-y-z/libsignal_dart/releases/tags/$tag',
    );
    final request = await client.getUrl(url);
    request.headers.set('Authorization', 'token $token');
    request.headers.set('Accept', 'application/vnd.github.v3+json');
    request.headers.set('User-Agent', 'libsignal-build-script');
